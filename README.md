...
# API (Client-side JS)
Put an iframe with the href <a href="https://yikuansun.github.io/sci-fi-flares/">https://yikuansun.github.io/sci-fi-flares/</a> (the iframe can be hidden if desired). Use `postMessage` on the iframe's `contentWindow`, the message's data being the preset array `[hue, hotspottype, hotspotscale, streaktype, streakscale, streakbalance, iristype, irisscale, seed, halotype, haloscale]`. The iframe window will post back a message with the data URL of the lens flare image.